...
class ProjectFrame(wx.Frame):
    ...
    def __init__(self):
        super(ProjectFrame, self).__init__(None, -1, '中文短文本分类系统',
                                           size=(530, 430))
        panel = wx.Panel(self, -1)
        rev = wx.StaticText(panel, -1, '中文短文本分类系统',
                            pos=(167, 5), size=(85, 25), style=wx.ALIGN_CENTER)
        font = wx.Font(15, wx.DECORATIVE, wx.ITALIC, wx.NORMAL)
        rev.SetFont(font)

        author = wx.StaticText(panel, -1, '',
    ...
    def process_programmer(self, urls):
        ...
        try:
            ...
            self.WritetoFront('无法连接数据库，请检查数据库设置!', False, 1)
            return
        ...
        if unknown_urls:
            ...
            if not startspider.check_linked():
                self.WritetoFront('无法连接网络，请检查网络库设置!', False, 1)
                return
            import PageExtract_Module.startextractor as startextractor
            import Classifier_Module.Filter as cfilter
            import Classifier_Module.DistributekNN as knnclassifier
            os.chdir('Spider_Module')
            (download_path, connect_web) = startspider.main(unknown_urls)
    ...
    def statis_result(self):
        if not self.result_set: return
        results = {};
        results_sum = len(self.result_set)
        for web, cateno in self.result_set:
...
def main():
    ...
    frame = ProjectFrame()
    progressMax = 21
    dialog = wx.ProgressDialog('中文短文本分类系统', '加载配置...', progressMax,
